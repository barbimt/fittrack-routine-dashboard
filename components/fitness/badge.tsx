...
type FitnessBadgeVariant = "default" | "soft" | "muscle" | "success" | "warning";
...
export function Badge({ className, variant = "default", ...props }: FitnessBadgeProps) {
  if (variant === "default") {
  ...
  return (
    <UiBadge
      className={cn("font-medium border", variantClasses[variant], className)}
      {...props}
    />
  );
}
